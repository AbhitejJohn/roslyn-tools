...
{{
    ...
    ""base"": ""{destBranch}""
        ...
        /// <summary>
        /// Merges a PR if all tests have passed and it has the label with
        /// text <see cref="AutoMergeLabelText" /> attached.
        /// </summary>
        /// <returns>
        /// (true, null) if successfully merged.
        /// (false, null) if the PR was not eligible.
        /// (false, non-null) if a Github-related error occurred while checking
        /// eligibility or attempting to merge an eligible PR.
        /// </returns>
        public async Task<(bool merged, HttpResponseMessage error)> MergePrIfEligible(
            string repoOwner,
            string repoName,
            string prNumber)
        {
            // Check the PR is open and mergeable
            var prInfoResponse = await _client.GetAsync($"repos/{repoOwner}/{repoName}/pulls/{prNumber}");

            if (!prInfoResponse.IsSuccessStatusCode)
            {
                return (false, prInfoResponse);
            }

            var prBody = JObject.Parse(await prInfoResponse.Content.ReadAsStringAsync());
            if (prBody["state"].Value<string>() != "open" ||
                prBody["mergeable"].Value<bool?>() != true)
            {
                return (false, null);
            }
            ...
            // Check the PR has a well-known 'auto-merge' label
            var prIssueResponse = await _client.GetAsync($"repos/{repoOwner}/{repoName}/issues/{prNumber}");
            ...
            // Check that there are no rejections
            var prReviewsResponse = await _client.GetAsync($"repos/{repoOwner}/{repoName}/pulls/{prNumber}/reviews");
            if (!prReviewsResponse.IsSuccessStatusCode)
            {
                return (false, prReviewsResponse);
            }

            var prReviews = JArray.Parse(await prReviewsResponse.Content.ReadAsStringAsync());
            ...
            // Merge the PR
//            var prMergeResponse = await _client.PutAsyncAsJson($"repos/{repoOwner}/{repoName}/pulls/{prNumber}/merge", $@"
//{{
//    ""merge_method"": ""merge""
//}}");

//            if (!prMergeResponse.IsSuccessStatusCode)
//            {
//                return (false, null);
//            }
